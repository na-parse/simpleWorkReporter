...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog='swr start')
    parser.add_argument(
        '-b', '--bind',
        metavar='HOST',
    ...
    if not paths.config_path().exists() or not settings.is_complete:
        ...
        print('Run ./swr setup first.', file=sys.stderr)
    ...
    try:
        ...
        print('Run ./swr db init to repair, or correct the data directory.', file=sys.stderr)
        return 1
...
def _prepare_ssl(settings: WorkerSettings):
    ...
    if not is_ssl_configured():
        ...
        print('Run ./swr cert regen to replace them, or ./swr cert delete + ', file=sys.stderr)
        print('./swr setup to disable HTTPS.', file=sys.stderr)
        return False
